    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ isset($evenement) ? 'Modifier l’événement' : 'Créer un nouvel événement' }}</h3>
            </div>
            <div class="block-content">
                ...
                <form id="formEvenement"
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" class="form-control" id="description" rows="3" required>{{ old('description', $evenement->description ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="datetime-local" name="date_debut" class="form-control" id="date_debut"
                                   value="{{ old('date_debut', isset($evenement) ? \Carbon\Carbon::parse($evenement->date_debut)->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="datetime-local" name="date_fin" class="form-control" id="date_fin"
                                   value="{{ old('date_fin', isset($evenement) ? \Carbon\Carbon::parse($evenement->date_fin)->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}" required>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="btn mb-2 {{ isset($evenement) ? 'btn-success' : 'btn-primary' }}">
            ...
            $('#image').on('change', function() {
                ...
                if (input.files && input.files[0]) {
                    ...
                    reader.onload = function(e) {
                        $('#preview-container').html('<img src="'+e.target.result+'" class="img-fluid rounded border p-1" style="max-height:200px;">');
                    }
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            let messages = Object.values(errors).map(msg => msg[0]).join('<br>');

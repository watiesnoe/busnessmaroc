                ...
                <div class="col-md-6 mb-4">
                    ...
                    <select name="category_id" class="form-select" required>
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}"
                                {{ old('category_id', $immobilier->category_id ?? '') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-4">
                    ...
                    <select name="statut" class="form-select">
                        <option value="disponible"
                            {{ old('statut', $immobilier->statut ?? '') == 'disponible' ? 'selected' : '' }}>Disponible
                        </option>
                        <option value="reserve"
                            {{ old('statut', $immobilier->statut ?? '') == 'reserve' ? 'selected' : '' }}>Réservé
                        </option>
                        <option value="loue"
                            {{ old('statut', $immobilier->statut ?? '') == 'loue' ? 'selected' : '' }}>Loué</option>
                    </select>
                </div>
                    ...
                    <tbody id="chambres-body">
                        @if (isset($immobilier))
                            @foreach ($immobilier->chambres as $index => $chambre)
                                <tr>
                                    <td>
                                        <select name="chambres[{{ $index }}][type]" class="form-control"
                                            required>
                                            <option value="chambre_simple"
                                                {{ $chambre->type == 'chambre_simple' ? 'selected' : '' }}>Chambre
                                                simple</option>
                                            <option value="chambre_double"
                                                {{ $chambre->type == 'chambre_double' ? 'selected' : '' }}>Chambre
                                                double</option>
                                            <option value="studio" {{ $chambre->type == 'studio' ? 'selected' : '' }}>
                                                Studio</option>
                                            <option value="suite" {{ $chambre->type == 'suite' ? 'selected' : '' }}>
                                                Suite</option>
                                            <option value="chambre_partagee"
                                                {{ $chambre->type == 'chambre_partagee' ? 'selected' : '' }}>Chambre
                                                partagée</option>
                                            <option value="dortoir"
                                                {{ $chambre->type == 'dortoir' ? 'selected' : '' }}>Dortoir</option>
                                            <option value="chambre_deluxe"
                                                {{ $chambre->type == 'chambre_deluxe' ? 'selected' : '' }}>Chambre
                                                deluxe</option>
                                            <option value="chambre_familiale"
                                                {{ $chambre->type == 'chambre_familiale' ? 'selected' : '' }}>Chambre
                                                familiale</option>
                                            <option value="appartement"
                                                {{ $chambre->type == 'appartement' ? 'selected' : '' }}>Appartement
                                            </option>
                                            <option value="bungalow"
                                                {{ $chambre->type == 'bungalow' ? 'selected' : '' }}>Bungalow</option>
                                            <option value="villa" {{ $chambre->type == 'villa' ? 'selected' : '' }}>
                                                Villa</option>
                                            <option value="mezzanine"
                                                {{ $chambre->type == 'mezzanine' ? 'selected' : '' }}>Mezzanine
                                            </option>
                                        </select>
                                    </td>
                                    <td><input type="number" name="chambres[{{ $index }}][capacite]"
                                            class="form-control" value="{{ $chambre->capacite }}" required></td>
                                    <td>
                                        <input type="number" name="chambres[{{ $index }}][prix_jour]"
                                            class="form-control mb-1" placeholder="Jour"
                                            value="{{ $chambre->prix_jour }}">
                                        <input type="number" name="chambres[{{ $index }}][prix_mois]"
                                            class="form-control mb-1" placeholder="Mois"
                                            value="{{ $chambre->prix_mois }}">
                                        <input type="number" name="chambres[{{ $index }}][prix_annee]"
                                            class="form-control" placeholder="Année"
                                            value="{{ $chambre->prix_annee }}">
                                    </td>
                                    <td>
                                        <select name="chambres[{{ $index }}][statut]" class="form-control">
                                            <option value="disponible"
                                                {{ $chambre->statut == 'disponible' ? 'selected' : '' }}>Disponible
                                            </option>
                                            <option value="reservee"
                                                {{ $chambre->statut == 'reservee' ? 'selected' : '' }}>Réservée
                                            </option>
                                            <option value="occupee"
                                                {{ $chambre->statut == 'occupee' ? 'selected' : '' }}>Occupée</option>
                                        </select>
                                    </td>
                                    <td>
                                        <textarea name="chambres[{{ $index }}][description]" class="form-control">{{ $chambre->description }}</textarea>
                                    </td>
                                    <td><button type="button" class="btn btn-danger btn-sm remove-chambre">X</button>
                                    </td>
                                </tr>
                            @endforeach
                        @endif
